  ...
  if (autoComplete.length > 0) {
    autoComplete.forEach((complete) => {
      ...
      if (inputDisplay && inputFilter) {
        ...
        // Crear opción "Todos" y asegurar que esté al principio
        ...
        opciones.forEach((op) => {
          ...
          op.addEventListener("click", () => {
            ...
            // Checkear si todas están seleccionadas
            const total = opciones.filter(op => op.id !== "all").length;
            const selected = opciones.filter(op => op.classList.contains("select") && op.id !== "all").length;
            allOption.classList.toggle("select", total === selected);
